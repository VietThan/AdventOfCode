...
santas_pos = (0, 0)
robot_santas_pos = (0, 0)

houses = {santas_pos : 2}

for i, c in enumerate(input, start=1):

    this_move = moves.get(c, None)

    if this_move is None:
        continue

    is_real_santa = True if i % 2 == 1 else False

    x_delta, y_delta = this_move
    x, y = santas_pos if is_real_santa else robot_santas_pos
    x_n, y_n = x + x_delta, y + y_delta
    new_pos = (x_n, y_n)
    if new_pos not in houses:
        houses[new_pos] = 0

    if is_real_santa:
        santas_pos = new_pos
    else:
        robot_santas_pos = new_pos
